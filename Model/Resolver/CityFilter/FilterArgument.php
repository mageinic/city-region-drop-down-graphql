...
namespace MageINIC\CityRegionPostcodeGraphQl\Model\Resolver\CityFilter;
...
class FilterArgument implements FieldEntityAttributesInterface
{
    ...
    {
        $cityFilterType = $this->config->getConfigElement('CityInfo');

        if (!$cityFilterType) {
            throw new \LogicException((string)__("City type not defined in schema."));
        }

        $fields = [];
        foreach ($cityFilterType->getFields() as $field) {
