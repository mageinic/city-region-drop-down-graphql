...
namespace MageINIC\CityRegionPostcodeGraphQl\Model\Resolver\PostcodeFilter;
...
class FilterArgument implements FieldEntityAttributesInterface
{
    ...
    {
        $postcodeFilterType = $this->config->getConfigElement('PostcodeInfo');

        if (!$postcodeFilterType) {
            throw new \LogicException((string)__("Postcode type not defined in schema."));
        }

        $fields = [];
        foreach ($postcodeFilterType->getFields() as $field) {
